...
export default class SelectStateView extends Component {
    ...
    componentWillMount() {
        ...
        for (let j = 0; j < AllLetters.length; j++) {
            ...
            for (let i = 0; i < this.AllState.length; i++) {
                if (AllLetters[j] === this.AllState[i].substr(0, 1)) {
                    each.push(this.AllState[i]);
                    flag = true;
                }
            }
            ...
            if (flag) {
                ...
                let _state = {};
                _state.index = this.letters.length - 1;
                _state.key = AllLetters[j];
                _state.data = each;
                this.states.push(_state)
            }
        }

        this.setState({data: this.states});
    }
    ...
    searchContentChange(searchKeywords) {
        if (searchKeywords === '') {
            ...
            let arr = [];
            for (let i = 0; i < this.AllState.length; i++) {
                if (this.AllState[i].toUpperCase().startsWith(searchKeywords.toUpperCase())) {
                    arr.push(this.AllState[i]);
                }
            }
    ...
    _getItemLayout(data, index) {
        let [length, header] = [ROW_HEIGHT, SECTION_HEIGHT];
        return {length, offset: length * index + header, index};
    }
            ...
            <View style={{height: Dimensions.get('window').height, marginBottom: 10}}>
...

                {
                    this.isSearch ?
                        ...
                        <View style={styles.listView}>
                            <SectionList style={{backgroundColor: '#fff'}}
...
const styles = StyleSheet.create({
    ...
    inputText: {
        flex: 1,
        padding: 0, //TextInput在安卓上默认有一个底边框，同时会有一些padding。如果要想使其看起来和iOS上尽量一致，则需要设置padding: 0，同时设置underlineColorAndroid="transparent"来去掉底边框。
        backgroundColor: 'transparent',
        fontSize: px2dp(14)
    }
});